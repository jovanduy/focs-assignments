...
(only greedy; not recursive or DP (top down or bottom up))

b. If the text contains a word that is longer than `max_width`, what does this algorithm do?

That word goes on its own line (the whole word is on one line).

c. Does this algorithm produce an optimal result? (Do you have enough information to answer this question?)

This really depends on what is considered 'optimal' for this problem. If optimal means that the max width of a line is never exceeded, then yes, it produces an optimal result: lines are never too long (except when a word is longer than the max length, which is okay because the decision was made to give such words their own line instead of somehow breaking them up). However, this does not mean that it is a globally optimal way of doing it, rather it is just a possible optimal solution that meets the requirements.
...
Bottom up dynamic programming. The line costs are stored early on. 
